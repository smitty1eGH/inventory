...
class InventoryDatabase:
    def __init__(self, dsn: str):
        self.cnx = psycopg2.connect(
            dsn=dsn, cursor_factory=psycopg2.extras.RealDictCursor
        )
    ...
    def destroy(self):
        ...
        self._u(
            "DROP TABLE IF EXISTS sale_items, order_items, sales, orders, items, users, flags CASCADE"
        )
    ...
    def delete_item_from_order(self, params):
        ...
        self._u(
            "DELETE FROM order_items WHERE order_id = %(order_id)s AND item_id = %(item_id)s",
            params,
        )
    ...
    def delete_item_from_sale(self, params):
        ...
        self._u(
            "DELETE FROM sale_items WHERE sale_id = %(sale_id)s AND item_id = %(item_id)s",
            params,
        )
    ...
    def get_or_add_user(self, params):
        # params = {'user_email': 'user@example.com'}
        row = self._q_one(
            "SELECT user_id, user_email FROM users WHERE user_email = %(user_email)s",
            params,
        )
        if row is None:
            params["user_id"] = uuid.uuid4()
            self._u(
                "INSERT INTO users (user_id, user_email) VALUES (%(user_id)s, %(user_email)s)",
                params,
            )
            return params
        return row
    ...
    def migrate(self):
        ...
        if self.version == 0:
            ...
            self._u(
                "INSERT INTO flags (flag_name, flag_int) VALUES (%s, %s)",
                ["db_version", 1],
            )
    ...
    def version(self):
        ...
        if row is None:
            return 0
        row = self._q_one(
            "SELECT flag_int FROM flags WHERE flag_name = %s", ["db_version"]
        )
